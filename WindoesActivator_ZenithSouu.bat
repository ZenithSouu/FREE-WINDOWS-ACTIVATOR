...
::=====================================================================================================================
::                                                                                                                    |
::  Ce script est destiné aux personnes qui ne peuvent pas dépenser d'argent pour une clé d'activation pour Windows.  |
::                                                                                                                    |
::                                                                                                                    |
::                                                                                                                    |
::=====================================================================================================================
...
:: Relancer le script avec un processus x64 s'il a été lancé par un processus x86 sous Windows x64 bit
:: ou avec le processus ARM64 s'il a été lancé par un processus x86/ARM32 sous Windows ARM64
...
:: Relancer le script avec le processus ARM32 s'il a été lancé par un processus x64 sur Windows ARM64
...
::  Définir la variable Path, cela aide si elle est mal configurée dans le système.
...
cls
color 07
title  Activation de clée windows Made By ZenithSouu
...
%nceline%
echo Une version non compatible du système d'exploitation a été détectée.
echo Le projet n'est pris en charge que pour Windows 7/8/8.1/10/11 et leur équivalent serveur.
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo Impossible de trouver powershell.exe dans le système.
echo Abandon...
goto MASend
)
...
::  Correction de la limitation des caractères spéciaux dans le nom du chemin d'accès
...
%nceline%
echo Le script est lancé à partir du dossier temporaire,
echo Il est très probable que vous exécutiez le script directement à partir du fichier d'archive.
echo:
echo Extrayez le fichier d'archive et lancez le script à partir du dossier extrait.
goto MASend
)
)
...
::============================================================================
::
::   Ce script fait partie du projet 'Microsoft Activation Scripts Fais par ZenithSouu' (MAS).
::
::   
::
::============================================================================
...
REM echo:
REM if !_lock!==0 (
REM echo                 [G] Générer un ticket
REM ) else (
REM call :dk_color2 %_White% "                [G] Générer un ticket       " %_Yellow% "  [LockBox Method]"
REM )
REM echo                 ____________________________________________
REM echo:      
REM echo                 [C] Modifier la méthode
REM echo:
REM echo                 [0] %_exitmsg%
REM echo         ____________________________________________________________
REM echo: 
REM call :dk_color2 %_White% "              " %_Green% "Saisir une option de menu dans le clavier:"
REM choice /C:1GC0 /N
